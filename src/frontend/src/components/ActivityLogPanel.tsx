...
export function ActivityLogPanel() {
  ...
  return (
    <div className="glass-card panel-sharp rounded-xl p-5 space-y-4">
      ...
      {entries.length === 0 ? (
        <div className="panel-sharp rounded-xl border border-border/50 bg-muted/20 px-4 py-5 text-sm text-muted-foreground">
        ...
        <div className="space-y-3">
          {entries.slice(0, 6).map((entry) => (
            <div
              key={entry.id}
              className="panel-sharp rounded-xl border border-border/50 bg-background/40 px-4 py-3"
            >
